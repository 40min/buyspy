...
class TelegramService:
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            if not response_text:
                ...
                self.logger.warning(f"No content received from agent engine for message: {user_message[:100]}")
            
            self.logger.info(f"Sending response ({len(response_text)} chars) to chat {chat_id}")
            ...
            self.logger.error(f"Error handling message from user {update.effective_user.id}: {e}", exc_info=True)
            ...
            try:
                ...
                self.logger.error(f"Failed to send error message to chat {update.effective_chat.id}: {send_error}")
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to send welcome message to user {update.effective_user.id}: {e}")
